          ...
          {step === 'form' && (
            <div>
              ...
              <form onSubmit={handleInitialSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                {/* 📚 Selección de Colección si es categoría Tomos y Libros */}
                {category === 'tomos' && librosCollections.length > 0 && (
                  <div>
                    <label style={{ display: 'block', fontWeight: 700, fontSize: '0.8rem', marginBottom: '4px', color: 'var(--text-secondary)' }}>
                      📚 Colección o Serie de Libros (Opcional)
                    </label>
                    <select
                      value={selectedBookCollectionId}
                      onChange={(e) => setSelectedBookCollectionId(e.target.value)}
                      style={{
                        width: '100%',
                        padding: '10px 14px',
                        borderRadius: '14px',
                    ...
                    >
                      <option value="" style={{ background: 'var(--card-bg)', color: 'var(--text-main)' }}>
                        -- Ninguna / Libro Independiente --
                      </option>
                      {librosCollections.map(col => (
                        <option key={col.id} value={col.id} style={{ background: 'var(--card-bg)', color: 'var(--text-main)' }}>
                          {col.nombre} ({col.recursos?.length || 0} tomos)
                        </option>
                      ))}
                    </select>
                    <span style={{ fontSize: '0.74rem', color: 'var(--text-secondary)', display: 'block', marginTop: '3px' }}>
                      Si seleccionas una colección, este libro se agrupará directamente dentro de ella.
                    </span>
                  </div>
                )}
